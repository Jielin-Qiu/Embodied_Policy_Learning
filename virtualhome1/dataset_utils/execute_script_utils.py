...
import json
import cv2
from virtualhome1.simulation.evolving_graph.scripts import Script
from virtualhome1.dataset_utils import add_preconds
...
def parse_exec_script_file(file_name):
    ...
    for elem in script_raw:
        ...
        # script.append(re.sub('1.', '', elem))
        # script.append(elem)
        # print(elem)
        try:
            period_index = elem.index('.')
            par_index = elem.index('(')
            split_f = [*elem]
            split_f.remove('.')
            num = split_f[par_index + 1]
            split_f.remove(f'{num}')
            joined_remove = ''.join(split_f)
            script.append(joined_remove)
        except:
            script.append(elem)
...
def render_script(comm, script, init_graph, scene_num, render_args, preconds = None):
